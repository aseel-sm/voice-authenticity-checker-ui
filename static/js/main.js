  ...
  function getFileUrl(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function (e) {
        ...
        let p = "/static/audio/" + input.files[0].name;
        console.log(p);
        $("#audio-container").html(
          `<source  src='${p}'  type="audio/mpeg"> Your browser does not support the audio element.`
        );
        // $("#audio-src").attr("src", p);
        $("#result").html("<div></div>");
  ...
  $("#predict-btn").click(() => {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#imageWidget").attr(
          "src",
          "http://127.0.0.1:5001/static/images/test.png"
        );
        $("#caption").show();
        btn = "<button class='btn btn-success'>The Voice is Authentic</button>";
        if (data["status"] == 0) {
          btn =
            "<button class='btn btn-danger'>The Voice is Generated</button>";
        }
